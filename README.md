...
- **건강기록 캘린더**: `건강기록` 탭 상단에 월간 캘린더(`HealthCalendar.tsx`)가 있어 기록이 있는
  날짜에 점으로 표시되고, 날짜를 누르면 해당 기록을 바로 열어볼 수 있습니다. 우측 상단 **"캘린더
  연동"** 버튼을 누르면 전체 건강기록을 표준 iCalendar(.ics) 파일로 내보내는데, 구글 캘린더
  웹(calendar.google.com) → 설정 → **캘린더 가져오기**에서 이 파일을 열면 전체 일정이 한 번에
  등록됩니다. (Google Calendar API를 통한 실시간 자동 동기화는 별도 OAuth 캘린더 스코프·백엔드
  작업이 필요해 이번 범위에는 포함하지 않았습니다.)
...
- 건강기록 탭의 "캘린더 연동"(.ics 다운로드) → Artifact 미리보기 샌드박스는 페이지가 자체적으로
  시작하는 파일 다운로드를 차단하므로 동작하지 않습니다. 실제 배포본(Vercel 등)에서는 정상
  작동합니다.
